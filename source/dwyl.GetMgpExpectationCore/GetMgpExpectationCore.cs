...
namespace dwyl.GetMgpExpectationCore
{
    ...
    {
        public static IEnumerable<string> Calculate(string numbers)
        {
            ...
            var maxMean = resultsOfCalculations.OrderByDescending(x => x.Value.mean).First().Value.mean;
            var maxMedian = resultsOfCalculations.OrderByDescending(x => x.Value.median).First().Value.median;

            foreach (var (key, (mean, median)) in resultsOfCalculations)
            {
                yield return $"{key.PadRight(17)} {mean.ToString("F").PadLeft(8)}{(mean.Equals(maxMean) ? " *" : "  ")} {median.ToString("F").PadLeft(8)}{(median.Equals(maxMedian) ? " *" : "  ")}";
            }
        ...
        private static IEnumerable<byte[]> GetPermutationEnumerable(byte[] hiddenNumbers)
        {
    ...
    public static class MyExtensions
    {
        ...
        public static IEnumerable<KeyValuePair<string, (double mean, double median)>> ToKeyValuePairs(this (ushort topRow, ushort middleRow, ushort bottomRow, ushort leftColumn, ushort middleColumn, ushort rightColumn, ushort downwardSloping, ushort upwardSloping)[] resultsOfCalculations)
        {
            yield return new KeyValuePair<string, (double, double)>("Top Row", (resultsOfCalculations.Select(x => x.topRow).ToMeanAndMedian()));
            yield return new KeyValuePair<string, (double, double)>("Middle Row", resultsOfCalculations.Select(x => x.middleRow).ToMeanAndMedian());
            yield return new KeyValuePair<string, (double, double)>("BottomRow", resultsOfCalculations.Select(x => x.bottomRow).ToMeanAndMedian());
            yield return new KeyValuePair<string, (double, double)>("Left Column", resultsOfCalculations.Select(x => x.leftColumn).ToMeanAndMedian());
            yield return new KeyValuePair<string, (double, double)>("Middle Column", resultsOfCalculations.Select(x => x.middleColumn).ToMeanAndMedian());
            yield return new KeyValuePair<string, (double, double)>("Right Column", resultsOfCalculations.Select(x => x.rightColumn).ToMeanAndMedian());
            yield return new KeyValuePair<string, (double, double)>("Downward Sloping", resultsOfCalculations.Select(x => x.downwardSloping).ToMeanAndMedian());
            yield return new KeyValuePair<string, (double, double)>("Upward Sloping", resultsOfCalculations.Select(x => x.upwardSloping).ToMeanAndMedian());
        }
